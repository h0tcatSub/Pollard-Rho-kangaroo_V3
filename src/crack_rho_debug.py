...
class ECCSolver_secp256k1:
    ...
    def rho_init(self):
        # set p_T,p_alp,p_bet
        self.p_T = self.p_Q.add(self.p_R)           # T=Q+R on ECC 
        self.p_alp = 1
        self.p_bet = 1
        # set st_n,Tlim
        self.st_n = int(math.sqrt(self.G.order)*2)
        Tlim = self.p_Q.modulo
        st_ptr = np.zeros(self.st_size)
        if self.st_n > self.st_size:
            self.st_n = self.st_size
            self.Tlim = self.p_Q.modulo // 4
        # set st_ptr
        for k in range(self.st_n):             # all pointer = -1
            st_ptr[k] = -1
        return Tlim
    ...
    #=========================================================#
    def rho_step(self):
        ip = self.p_T.x % 3                   # ip=p_T[0] (mod 3)
        if (ip == 0):
            self.p_T *= self.p_T            # p_T = 2*p_T on ECC
    ...
    #=========================================================#
    def plus_Ty(self):
        list = [self.p_T.x, self.p_alp, self.p_bet]
        if self.p_T.y > (self.p_T.order >> 1):
            list[1] = self.p_T.order - self.p_alp
            list[2] = self.p_T.order - self.p_bet
        return list
    ...
    def sol_equal(self, loop, al, bl):
        Tab = self.plus_Ty()                 # get alp,bet : T[0]>0
        bval = Tab[2] - bl
        n = 0
        if bval != 0:
    ...
    #    date : 2020/01/30                                    #
    #=========================================================#
    def equal_ck(self, loop):
        #global st_Tab,st_ptr,st_n
        # check pointer equal
        pt = self.p_T.x % self.st_n                  # p_T[0] (mod st_n)
        stv = self.st_ptr[pt]
        Tab = self.plus_Ty()                     # get value : T[0]>0
        if stv >= 0:                        # check used table      
            if self.p_T.x == self.st_tab[stv][0]:
        ...
        # change st_ptr & add st_Tab
            ...
            self.st_ptr[pt] = len(self.st_tab)         # point last
            self.st_tab = np.append(self.st_tab, Tab)
        return 0
    ...
    def rho_method(self):
        ...
        lpmx = int(math.sqrt(self.G.order)*10)       # Rho loop max
        # Rho step
        outc = 0
        for lp in range(lpmx):
            outc += 1
            self.rho_step()
            if (self.p_T.x == 0) and (self.p_T.y == 0):                 # solve by T=zero
                self.sol_zero(lp+1) 
                return 2
            if self.p_T.x < Tlim:                # only feature poin
                id = self.equal_ck(lp+1)           # check T=old T 
                if id != 0:
        ...
        for k in range(nv):
            ...
            for k in range(len(self.p_Q)):
                if (self.p_Q[k].x == 0) and (self.p_Q[k].y == 0):
                    ...
                    if np.all(self.g_r % 2 != 0) and np.all(self.g_r % 3 != 0):
                        ...
                        try:
                            assert G * n == Y
                            print(f"Direct Solve. result : {format(n, '064x')}")
                        except:
                            keys = np.arange(n, n + 0x1000 + 1)
                            Gs = (G * keys)
                            assert Y in Gs
                            n = keys[Gs * keys == Y]#np.where((G * keys))
                        return n
