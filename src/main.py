...
def comparator():
    ...
    if len(result) > 0: 
        ...
        print (f'total time: {(time.time()-starttime)} sec')
...
def search():
    ...
    while True:
        Hops += 1
        pw = T.x #T[k].x % hop_modulo
        dt = 1 << pw
        solved = check(T, t, DP_rarity, "tame.txt")
        if solved: break
        t += dt
        T = add(P, T)
        if solved: break            
        ...
        pw = W.x % hop_modulo
        dw = 1 << pw
        solved = check(W, w, DP_rarity, "wild.txt")
        if solved: break
        w += dw
        W = add(P, W)
        if solved: break
