...
global starttime
starttime = time.time()
def kangaroo_algorithm(g, h, n, factors, B, bits_size):
    global starttime
    ...
    _d = np.array([])
    def calc_d(u, v, n):
        return gcd(u.x - v.y, n)
    calc_d_vec = np.vectorize(calc_d)
    kangaroo_point = Point(Point.Gx, Point.Gy)
    while True:
        for wild_x, wild_y in wild_kangaroos:
            ...
            if ((np.all(1 < _d) and np.all(_d < n))):
                    ...
                    return d
            if np.isin(_d, 1)[0]:
                alpha = kangaroo_attack_vec(P, Q, g, n, h, B, bits_size)  # Using kangaroo_attack
...
def main():
    ...
    Q = G * 10000#Point(x, y)
    bits_size = 2 ** 10 + 1#int(args.bits_size)
    B = bits_size - 1
    ...
    private_key = kangaroo_algorithm(G, Q, n, factors, B , bits_size)
    G = Point(Point.Gx, Point.Gy)
